...
- `WalletService` looks up the wallet with Spring Data's derived `findByUserId` method and defensively confirms the owner ID before mapping a DTO. No client-supplied `userId` or `walletId` participates in authorization.
